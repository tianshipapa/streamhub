...
const Home: React.FC<HomeProps> = ({ 
  ...
  const [mode, setMode] = useState<'SOURCE' | 'DOUBAN' | 'FAVORITE'>(savedState.isDoubanMode ? 'DOUBAN' : 'SOURCE');
  ...
  const handleClearFavs = () => {
    if (confirmClearFav) { clearFavorites(); setFavorites([]); setConfirmClearFav(false); }
    else { setConfirmClearFav(true); setTimeout(() => setConfirmClearFav(false), 3000); }
  };
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-2 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm">
             <div className="flex bg-gray-100 dark:bg-slate-900/50 p-1 rounded-xl w-full sm:w-auto">
                <button 
                    ...
                    className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${mode === 'SOURCE' ? 'bg-white dark:bg-slate-700 text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    <Icon name="dns" className="text-lg" />源站
                </button>
                <button 
                    onClick={() => { setMode('DOUBAN'); onStateUpdate({ isDoubanMode: true }); }}
                    className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${mode === 'DOUBAN' ? 'bg-pink-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-700'}`}
                >
                    <Icon name="auto_awesome" className="text-lg" />豆瓣
                </button>
                <button 
                    onClick={() => setMode('FAVORITE')}
                    className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${mode === 'FAVORITE' ? 'bg-amber-500 text-white shadow-lg' : 'text-gray-500 hover:text-gray-700'}`}
                >
             ...
             {mode === 'SOURCE' ? (
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                   <button onClick={() => onStateUpdate({ doubanType: 'movie', doubanMovies: [], doubanTag: '热门' })} className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-xs font-bold transition-all border ${savedState.doubanType === 'movie' ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 border-transparent' : 'text-gray-500 border-gray-200'}`}>电影</button>
                   <button onClick={() => onStateUpdate({ doubanType: 'tv', doubanMovies: [], doubanTag: '热门' })} className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-xs font-bold transition-all border ${savedState.doubanType === 'tv' ? 'bg-gray-900 dark:bg-white text-white dark:text-gray-900 border-transparent' : 'text-gray-500 border-gray-200'}`}>电视剧</button>
                </div>
             ) : (
